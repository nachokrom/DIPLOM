...
const getDisplayName = async () => {
    if (token.value) {
        try {
            ...
            const response = await axiosApiInstance.post(`https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=${apiKey}`, bodyPayload, config);
...
<template>
    <div>
        ...
        <div v-else class="relative">
            <div class="avatar_person px-1 cursor-pointer hover:bg-blue-700">
                <img
